...
document.addEventListener('DOMContentLoaded', function () {
  if (!window.__TAURI__ || !window.__TAURI__.core) {
    console.error('[SwiftPOS] Tauri API not available in main.js');
    return;
  }
  ...
  // State for the active configuration
  let currentAuthConfig = { ...DEFAULT_AUTH_CONFIG };

  /**
   * Initializes authentication configuration by checking multiple sources:
   * 1. window.ENV (injected during build or startup)
   * 2. Tauri backend via 'get_auth_config' command
   * 3. Static DEFAULT_AUTH_CONFIG as final fallback
   */
  const initAuthConfig = async () => {
    try {
      // Priority 1: Environment variables from window.ENV 
      ...
      // Priority 2: Fetch from Tauri backend
      ...
      console.warn('[Auth] Backend config unavailable, using defaults:', error.message);
    }

    // Final Fallback: defaults are already assigned to currentAuthConfig
    console.warn('[Auth] Using default fallback configuration');
  };

  // Global config object for external access (maintains existing API)
  const NEON_AUTH_CONFIG = {
    get projectId() { return currentAuthConfig.projectId; },
    get publishableClientKey() { return currentAuthConfig.publishableClientKey; }
  };
